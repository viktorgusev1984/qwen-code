...
This monorepo contains two main packages: `@psd-tech/gusqwen` and `@psd-tech/gusqwen-core`.

## `@psd-tech/gusqwen`

This is the main package for Gus Qwen. It is responsible for the user interface, command parsing, and all other user-facing functionality.

When this package is published, it is bundled into a single executable file. This bundle includes all of the package's dependencies, including `@psd-tech/gusqwen-core`. This means that whether a user installs the package with `npm install -g @psd-tech/gusqwen` or runs it directly with `npx @psd-tech/gusqwen`, they are using this single, self-contained executable.

## `@psd-tech/gusqwen-core`
...
Releases are managed through the [release.yml](https://github.com/GusQwen/gusqwen/actions/workflows/release.yml) GitHub Actions workflow. To perform a manual release for a patch or hotfix:
...
# Preview
npm install -g @psd-tech/gusqwen@preview

# Nightly
npm install -g @psd-tech/gusqwen@nightly
```
...
- `npx -y @psd-tech/gusqwen@latest --version` to validate the push worked as expected if you were not doing a rc or dev tag
- `npx -y @psd-tech/gusqwen@<release tag> --version` to validate the tag pushed appropriately
- _This is destructive locally_ `npm uninstall @psd-tech/gusqwen && npm uninstall -g @psd-tech/gusqwen && npm cache clean --force &&  npm install @psd-tech/gusqwen@<version>`
...
1.  Go to the [Actions tab](https://github.com/GusQwen/gusqwen/actions/workflows/release.yml) of the repository.
...
You can then inspect the generated tarballs to ensure that they contain the correct files and that the `package.json` files have been updated correctly. The tarballs will be created in the root of each package's directory (e.g., `packages/cli/gusqwen-0.1.6.tgz`).
...
- **Simplified Script Execution**: You can run scripts in any package from the root of the project using the `--workspace` flag. For example, to run the `build` script in the `cli` package, you can run `npm run build --workspace @psd-tech/gusqwen`.
