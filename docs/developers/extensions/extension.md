...
Gus Qwen extensions package prompts, MCP servers, and custom commands into a familiar and user-friendly format. With extensions, you can expand the capabilities of Gus Qwen and share those capabilities with others. They are designed to be easily installable and shareable.
...
```
qwen extensions install https://github.com/gusqwen-cli-extensions/security
```

This will install the Gus Qwen Security extension, which offers support for a `/security:analyze` command.
...
```
qwen extensions uninstall gusqwen-cli-security
```
...
We offer several example extensions `context`, `custom-commands`, `exclude-tools` and `mcp-server`. You can view these examples [here](https://github.com/GusQwen/gusqwen/tree/main/packages/cli/src/commands/extensions/examples).
...
On startup, Gus Qwen looks for extensions in `<home>/.qwen/extensions`
...
When Gus Qwen starts, it loads all the extensions and merges their configurations. If there are any conflicts, the workspace configuration takes precedence.
...
Gus Qwen extensions allow variable substitution in `qwen-extension.json`. This can be useful if e.g., you need the current directory to run an MCP server using `"cwd": "${extensionPath}${/}run.ts"`.
