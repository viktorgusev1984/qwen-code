# Getting Started with Gus Qwen Extensions

This guide will walk you through creating your first Gus Qwen extension. You'll learn how to set up a new extension, add a custom tool via an MCP server, create a custom command, and provide context to the model with a `QWEN.md` file.

## Prerequisites

Before you start, make sure you have the Gus Qwen installed and a basic understanding of Node.js and TypeScript.
...
This is the manifest file for your extension. It tells Gus Qwen how to load and use your extension.
...
- `mcpServers`: This section defines one or more Model Context Protocol (MCP) servers. MCP servers are how you can add new tools for the model to use.
  - `command`, `args`, `cwd`: These fields specify how to start your server. Notice the use of the `${extensionPath}` variable, which Gus Qwen replaces with the absolute path to your extension's installation directory. This allows your extension to work regardless of where it's installed.
...
Before you can use the extension, you need to compile the TypeScript code and link the extension to your Gus Qwen installation for local development.
...
3.  **Link the extension:**
    ...
    The `link` command creates a symbolic link from the Gus Qwen extensions directory to your development directory. This means any changes you make will be reflected immediately without needing to reinstall.

    ```bash
    qwen extensions link .
    ```

Now, restart your Gus Qwen session. The new `fetch_posts` tool will be available. You can test it by asking: "fetch posts".
...
After saving the file, restart the Gus Qwen. You can now run `/fs:grep-code "some pattern"` to use your new command.
...
Restart Gus Qwen again. The model will now have the context from your `QWEN.md` file in every session where the extension is active.
...
You've successfully created a Gus Qwen extension! You learned how to:
...
From here, you can explore more advanced features and build powerful new capabilities into the Gus Qwen.
