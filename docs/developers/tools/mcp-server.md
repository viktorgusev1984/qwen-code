# MCP servers with Gus Qwen

This document provides a guide to configuring and using Model Context Protocol (MCP) servers with Gus Qwen.
...
Gus Qwen integrates with MCP servers through a sophisticated discovery and execution system built into the core package (`packages/core/src/tools/`):
...
Gus Qwen uses the `mcpServers` configuration in your `settings.json` file to locate and connect to MCP servers. This configuration supports multiple servers with different transport mechanisms.
...
Gus Qwen supports OAuth 2.0 authentication for remote MCP servers using SSE or HTTP transports. This enables secure access to MCP servers that require authentication.
...
When Gus Qwen starts, it performs MCP server discovery through the following detailed process:
...
When Gus Qwen receives this response, it will:
...
In addition to tools, MCP servers can expose predefined prompts that can be executed as slash commands within Gus Qwen. This allows you to create shortcuts for common or complex queries that can be easily invoked by name.
...
```bash
/poem-writer --title="Gus Qwen" --mood="reverent"
```

or, using positional arguments:

```bash
/poem-writer "Gus Qwen" reverent
```
