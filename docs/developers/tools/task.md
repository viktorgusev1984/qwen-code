...
This document describes the `task` tool for Gus Qwen.
...
You can view available subagents by using the `/agents` command in Gus Qwen.
...
Subagents are configured through Gus Qwen's agent configuration system. Use the `/agents` command to:
