...
Gus Qwen supports a variety of themes to customize its color scheme and appearance. You can change the theme to suit your preferences via the `/theme` command or `"theme":` configuration setting.

## Available Themes

Gus Qwen comes with a selection of pre-defined themes, which you can list using the `/theme` command within the CLI:
...
Selected themes are saved in Gus Qwen's [configuration](../configuration/settings) so your preference is remembered across sessions.

---

## Custom Color Themes

Gus Qwen allows you to create your own custom color themes by specifying them in your `settings.json` file. This gives you full control over the color palette used in the CLI.
...
- Select your custom theme using the `/theme` command in Gus Qwen. Your custom theme will appear in the theme selection dialog.
