# Gus Qwen Companion Plugin: Interface Specification

> Last Updated: September 15, 2025

This document defines the contract for building a companion plugin to enable Gus Qwen's IDE mode. For VS Code, these features (native diffing, context awareness) are provided by the official extension ([marketplace](https://marketplace.visualstudio.com/items?itemName=gusqwenlm.gusqwen-vscode-ide-companion)). This specification is for contributors who wish to bring similar functionality to other editors like JetBrains IDEs, Sublime Text, etc.

## I. The Communication Interface

Gus Qwen and the IDE plugin communicate through a local communication channel.
...
For Gus Qwen to connect, it needs to discover what port your server is using. The plugin **MUST** facilitate this by creating a "lock file" and setting the port environment variable.
...
**Legacy note:** For extensions older than v0.5.1, Gus Qwen may fall back to reading JSON files in the system temp directory named `gusqwen-ide-server-<PID>.json` or `gusqwen-ide-server-<PORT>.json`. New integrations should not rely on these legacy files.
