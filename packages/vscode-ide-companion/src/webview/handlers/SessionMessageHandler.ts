...
export class SessionMessageHandler extends BaseMessageHandler {
  ...
  async handle(message: { type: string; data?: unknown }): Promise<void> {
    ...
    switch (message.type) {
      ...
      case 'openNewChatTab':
        ...
        try {
          await vscode.commands.executeCommand('gusqwen.openNewChatTab');
        } catch (error) {
          console.error(
  ...
  private async promptLogin(message: string): Promise<boolean> {
    ...
    if (result === 'Login Now') {
      if (this.loginHandler) {
        ...
        await vscode.commands.executeCommand('gusqwen.login');
      }
      return true;
    }
    return false;
  ...
  private async handleSendMessage(
    ...
    if (!this.agentManager.isConnected) {
      ...
      await this.promptLogin('You need to login first to use Gus Qwen.');
      return;
    }
      ...
      try {
        ...
        if (
          ...
          await this.promptLogin(
            'Your login session has expired or is invalid. Please login again to continue using Gus Qwen.',
          );
          return;
        }
